...
async def register(vk_id: int, name: str):
    ...
    if is_in_db:
        # Убираем "Пользователь XX" из БД, если фронтенд прислал реальное имя
        if is_in_db['name'] != name and not is_in_db['name'].startswith(name):
            await db.update_user_name(vk_id, name)
            is_in_db = await db.get_user_by_vk_id(vk_id) # Обновляем локальные данные
...
async def load_users_family(vk_id: int):
    ...
    if has_family:
        ...
        return {
            ...
            "family_name": family_data['name'] if family_data else "Семья" # Возвращаем имя семьи
        }
    else:
        return {
